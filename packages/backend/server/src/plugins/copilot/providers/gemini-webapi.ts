...
export class GeminiWebAPIProvider extends CopilotProvider<GeminiWebAPIConfig> {
  ...
  private readonly logFile =
    'c:\\Users\\admin\\ProjectsIT\\personal\\AFFiNE\\gemini-debug.log';
  ...
  protected override setup(): void {
    this.logDebug('🚀 GeminiWebAPIProvider.setup() called');
    super.setup();
    if (!this.configured()) {
      this.logDebug('❌ Not configured');
      return;
    }
    void this.initializeBridge().catch(err => {
      this.logDebug('❌ initializeBridge failed:', err);
      this.instance = null;
    });
  }
  ...
  private async initializeBridge(): Promise<void> {
    ...
    this.logDebug('🔌 Connecting to bridge at:', this.config.baseURL);
    ...
    try {
      ...
      if (!response.ok) {
        this.logDebug('❌ Health check failed:', response.status);
        throw new Error(
      ...
      this.logDebug('✅ Health check passed', data);
      ...
      this.logger.warn(
        `Failed to connect to Gemini WebAPI bridge service at ${this.config.baseURL}. Provider will be disabled.`
      );
      // Don't throw - just disable the provider gracefully
      this.instance = null;
    }
  }
  ...
  override async text(
    ...
    this.logDebug('🚀 text() called', { model: cond });
    ...
    this.logger.log(
      `🚀 [GEMINI WEBAPI] text() called - Model: ${model.id}, Messages:`,
      JSON.stringify(
        messages.map(m => ({
          role: m.role,
          content:
  ...
  override async *streamText(
    ...
    this.logDebug('🚀 streamText called', {
      model: cond,
      optionsTools: options.tools,
    });

    if (!this.instance) {
      this.logDebug('❌ Instance not initialized');
      throw new CopilotProviderNotSupported({
        provider: this.type,
        kind: 'text',
      });
    ...
    this.logDebug('🔍 Raw tools from getTools:', Object.keys(tools));

    // Filter out webSearch if present (per user request)
    if (tools.web_search_exa) {
      this.logDebug('Removing web_search_exa');
      delete tools.web_search_exa;
    }
    if (tools.web_crawl_exa) {
      this.logDebug('Removing web_crawl_exa');
      delete tools.web_crawl_exa;
    }

    const hasTools = Object.keys(tools).length > 0;
    this.logDebug('Final tools to pass:', Object.keys(tools));

    if (hasTools) {
      this.logger.log(
        `🔧 [GEMINI WEBAPI] Tools enabled: ${Object.keys(tools).join(', ')}`
      );
    }

    try {
      this.logDebug('Calling streamText with tools:', hasTools);
      const { streamText } = await import('ai');
      ...
      this.logDebug('❌ Error in streamText:', e);
      throw this.handleError(e);
    }
  }
