-- Create user ivan.en@affine.com and add to workspace
...
DECLARE
    ...
    v_new_user_email VARCHAR := 'ivan.en@affine.com';
    v_new_user_name VARCHAR := 'Ivan';
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM workspaces WHERE id = v_workspace_id) THEN
        ...
        INSERT INTO workspaces (
            ...
            NOW(),
            false, -- indexed
            NOW(), -- last_check_embeddings
            true,  -- enable_url_preview (defaulting to true)
            true,  -- enable_ai (defaulting to true given context)
            true   -- enable_doc_embedding (defaulting to true)
        );
        RAISE NOTICE 'Workspace created.';
    ELSE
        RAISE NOTICE 'Workspace % already exists.', v_workspace_id;
    END IF;
...
-- Verify the addition
SELECT u.email, u.name, wup.type, wup.accepted, w.public as workspace_public
FROM workspace_user_permissions wup
JOIN users u ON u.id = wup.user_id
JOIN workspaces w ON w.id = wup.workspace_id
WHERE wup.workspace_id = '7ed50eac-95f3-4c13-a16d-af46ba7dfc7a' AND u.email = 'ivan.en@affine.com';
